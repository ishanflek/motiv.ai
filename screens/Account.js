  ...
  return (
      <View className="bg-stone-900">
        <Appbar.Header style={{ backgroundColor: '#1D1D1D', width: '100%'}}>
          ...
          <Text style={{backgroundColor: "lightgrey", color: "black", fontSize: 22 / fs, height: 57, padding: 5}} className="w-full">
          {folders.length > 0 ? `${folders[0]}` : null}
          </Text>
          <Text style={{backgroundColor: "lightgrey", color: "black", fontSize: 20 / fs, height: 57, padding: 5, marginTop: 20}} className="w-full">
          {`${number}`}
          </Text>
